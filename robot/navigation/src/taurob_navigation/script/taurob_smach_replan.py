...
class Replanning(smach.Concurrence):
    ...
    def __init__(self):
        smach.Concurrence.__init__(
            ...
            input_keys=["target_pose"],
            output_keys=["target_pose", "outcome", "message", "path"],
            outcome_map={
                "received_goal": {"WAIT_FOR_GOAL": "received_goal"},
                "succeeded": {"PLANNING": "succeeded"},
                "preempted": {"WAIT_FOR_GOAL": "preempted", "PLANNING": "preempted"},
                "failure": {"PLANNING": "failure"},
                "invalid": {"PLANNING": "invalid"},
    ...
    def __init__(self):
        ...
        with self:
            smach.StateMachine.add(
                "WAIT_FOR_GOAL",
                WaitForGoal(),
                transitions={"received_goal": "REPLANNING", "preempted": "preempted"},
            )
            smach.StateMachine.add(
                "REPLANNING",
                Replanning(),
...
class DriveToGoal(smach.Concurrence):
    @staticmethod
    def child_term_cb(outcome_map):
        return True
    ...
    def __init__(self):
        smach.Concurrence.__init__(
            ...
            output_keys=["target_pose", "path"],
            outcome_map={
                "succeeded": {"CONTROLLING": "succeeded"},
                "preempted": {"PLAN_TO_GOAL": "preempted", "CONTROLLING": "preempted"},
                "received_path": {"PLAN_TO_GOAL": "succeeded"},
                "failure": {"CONTROLLING": "failure"},
                "aborted": {"CONTROLLING": "aborted"},
                "change_controller": {"CONTROLLING": "change_controller"},
            },
            child_termination_cb=DriveToGoal.child_term_cb,
        )

        with self:
            smach.Concurrence.add("PLAN_TO_GOAL", PlanToGoal())
            smach.Concurrence.add("CONTROLLING", Controlling())
...
def main():
    ...
    with SM:
        smach.StateMachine.add(
            "PLAN_TO_GOAL",
            PlanToGoal(),
            transitions={"succeeded": "DRIVE_TO_GOAL", "preempted": "preempted"},
        )
        ...
        smach.StateMachine.add(
            ...
            transitions={
                ...
                "preempted": "preempted",
                "received_path": "DRIVE_TO_GOAL",
                "failure": "RECOVERY",
                "aborted": "PLAN_TO_GOAL",
                "change_controller": "DRIVE_TO_GOAL",
            },
        )
    ...
    sis = smach_ros.IntrospectionServer(
        "mbf_state_machine_server", SM, "/MBF_SM_REPLANNING"
    )
    sis.start()
